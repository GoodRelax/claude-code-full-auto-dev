...
- [第4章 エージェント定義](#第4章-エージェント定義)
  - 4.1 エージェント定義の基本構造
  - 4.2 コアエージェント（architect / srs-writer / security-reviewer / test-engineer / review-agent / progress-monitor）
...
- [第7章 ほぼ全自動開発のワークフロー](#第7章-ほぼ全自動開発のワークフロー)
  ...
  - 7.3 Phase 1: 企画 — コンセプトからSRS作成
  - 7.4 Phase 2: 設計 — SWS・セキュリティ・WBS
...
- 要求仕様(SRS)作成、セキュリティ設計、SW仕様(SWS)作成
...
flowchart TB
    ...
    SRS_Agent -->|"6 仕様作成"| Arch_Agent
    Arch_Agent -->|"7 設計伝達"| Impl_Agent
...
project_root/
  ...
  .claude/
    agents/                       ... カスタムエージェント定義
      architect.md                ... アーキテクト（SWS設計）エージェント
      srs-writer.md               ... 要求仕様エージェント
...
- **Review Agent**: docs/reviews/ にレビュー報告を出力する。R1〜R6の観点でレビューし、Critical/High指摘がゼロになるまで次フェーズへの移行をブロックする
...
成果物の種類（SRS / SWS / コード）に応じた観点でレビューを実施し、
...
|------|-------------------|
| SRS（要求仕様書） | R1: 要件品質 |
| SWS・設計文書 | R2: 設計原則, R4: 並行性・状態遷移（設計）, R5: パフォーマンス（設計） |
...
|-----------|------|------|--------------|
| Phase 1 完了後（SRS承認前） | SRS | R1 | SRS修正（Phase 1へ戻る） |
| Phase 2 完了後（実装開始前） | SWS・設計 | R2, R4, R5（設計レベル） | SWS修正（Phase 2へ戻る） |
...
- R1指摘（要件品質）: SRS修正が必要 → Phase 1相当の作業
- R2/R4/R5指摘（設計レベル）: SWS修正が必要 → Phase 2相当の作業
...
SRS承認後に発生するすべての要件・設計変更を制御するプロセス。
...
要件IDからテストケースIDまでの双方向トレーサビリティを維持する。srs-writerが要件IDを付与し、architectが設計IDを付与し、test-engineerがテスト実装時に更新する。
...
- 命名規則: `{文書名}-v{メジャー}.{マイナー}.md`（例: `SRS-v1.2.md`）
- SRS承認時・重大な仕様変更時: メジャーバージョンをインクリメント
...
|----------|----------------------------------------|---------|
| **法規調査** | ・個人情報・個人データを扱う<br>・医療・ヘルスケア分野<br>・金融・決済処理を扱う<br>・通信サービスを提供する<br>・EU市場向けに提供する<br>・公共・行政向けシステム | Phase 0 SRS作成**前**<br>（最優先で判断） |
...
| **機能安全（HARA/FMEA）** | ・人命・身体への直接的影響がある（医療機器・車載・産業機器）<br>・IEC 61508 / ISO 26262 / IEC 62304 等への準拠が要求される<br>・社会インフラへの重大な影響がある<br>・金融基幹系で重大な資産損害リスクがある | Phase 0 コンセプト提示時<br>（**最優先で判断**、安全要件はSRS前に確定） |
| **アクセシビリティ（WCAG 2.1）** | ・Webアプリケーションを提供する<br>・EU市場向け（EAA指令 2025年6月完全施行）<br>・公共・行政向けシステム<br>・多様なユーザー層を対象とする | Phase 0 SRS作成**前**<br>（NFRとしてSRSに含める） |

#### 6.4.2 Phase 0 評価プロセス

full-auto-dev コマンドのPhase 0で、リードエージェントが以下を自動評価する（第5章5.1参照）。

1. 機能安全 → 該当する場合は**即座にユーザーに確認**を求め、SRS作成前に安全要件を確定する。`docs/safety/` を作成し、Phase 2 に HARA・FMEAを追加する
2. 法規調査 → 該当する場合はCLAUDE.mdに追記し、SRSの非機能要件セクションに規制要件を含める。`docs/legal/` を作成する
...
5. アクセシビリティ → 該当する場合はSRSのNFRにWCAG 2.1 AA準拠要件を追加し、review-agentのR1チェック項目に含める
...
flowchart LR
    subgraph Phase0["Phase 0<br/>プロセス評価"]
        P0A["AI:<br/>spec.mdバリデーション<br/>条件付きプロセス評価"]
        P0B["ユーザー:<br/>確認・承認"]
    end
...
Phase 0は全自動開発の**最初のステップ**であり、SRS作成前に必ず実行する。

**spec.mdバリデーション**として、以下の必須項目の記載を確認する:
- 解決したい課題、ターゲットユーザー、主要機能、制約条件、品質要求

不足項目があればユーザーに確認を求めてから先へ進む。バリデーション通過後、第6章6.4の判断基準に基づいて条件付きプロセスの要否を評価する。
...
ユーザーは `spec.md` にプロジェクトのコンセプトを記述する。以下の情報が含まれていると効率的である。

**spec.md の必須記載事項（Phase 0でバリデーション対象）:**
...
#### 7.3.2 AI のアクション: SRS 自動生成

```bash
claude "spec.mdを読み込み、ほぼ全自動開発を開始してください。
まずSRS(要求仕様書)をdocs/srs/SRS-v1.0.mdに作成してください。
作成後、review-agentでR1観点のレビューを実施し、
PASSしたらSRSの概要を報告し、重要な判断が必要な箇所があれば提示してください。"
```

#### 7.3.3 ユーザーの判断ポイント: SRS承認
...
SRSが承認されたら、設計フェーズを自動開始する。

```bash
claude "SRSが承認されました。以下を並列で実行してください:
1. docs/sws/SWS-v1.0.md にソフトウェア仕様書を作成
2. docs/api/openapi.yaml にOpenAPI 3.0仕様を生成
3. docs/security/ にセキュリティ設計を作成
4. docs/observability/observability-design.md に可観測性設計を作成
5. docs/progress/wbs.md にWBSとガントチャートを作成
6. risk-managerでリスク台帳を作成
7. review-agentでR2/R4/R5観点の設計レビューを実施し、PASSしたら次へ進む
...
gantt
    ...
    条件付きプロセス評価           : done, p0b, after p0a, 1d

    section Phase1_企画
    SRS作成                       : done, p1b, after p0b, 2d
    SRSレビュー(R1)               : done, p1c, after p1b, 1d
    SRS承認                       : milestone, p1d, after p1c, 0d

    section Phase2_設計
    SWS作成                       : p2a, after p1d, 3d
...
```bash
claude "SWSに基づき、Agent Teamsで並列実装を開始してください。
...
claude --worktree -p "SWSに基づきモジュールA（認証モジュール）を実装してください。
feature/auth-module ブランチで作業し、完了後 develop へのPRを作成してください"

# ターミナル2: モジュールB（feature/dashboard-module ブランチ）
claude --worktree -p "SWSに基づきモジュールB（ダッシュボード）を実装してください。
...
3. 性能テスト: SRSのNFR数値目標（レスポンスタイム・同時接続数等）に基づきk6シナリオを実行する
...
export const options = {
  // SRS NFR-002: 同時接続100ユーザーでレスポンス200ms以内
  stages: [
...
{
  "test_progress": [
    {"date": "2026-03-05", "planned": 50, "executed": 12, "passed": 10, "failed": 2},
    {"date": "2026-03-06", "planned": 50, "executed": 28, "passed": 25, "failed": 3},
    {"date": "2026-03-07", "planned": 50, "executed": 45, "passed": 42, "failed": 3},
    {"date": "2026-03-08", "planned": 50, "executed": 50, "passed": 48, "failed": 2}
  ]
}
```
...
- R1指摘 → SRS修正（Phase 1相当）
- R2/R4/R5設計指摘 → SWS修正（Phase 2相当）
...
flowchart TD
    SRS["SRS 作成完了"]
    SRS_Review{"R1レビュー<br/>PASS?"}
    SWS["SWS+OpenAPI+可観測性設計 完了"]
    SWS_Review{"R2/R4/R5レビュー<br/>PASS?"}
    Impl["実装+SCA完了"]
    ...
    Final["最終R1-R6レビュー"]
    R1Fail["SRS修正<br/>Phase 1相当"]
    R2Fail["SWS修正<br/>Phase 2相当"]
    R3Fail["コード修正<br/>Phase 3相当"]
    R6Fail["テスト修正<br/>Phase 4相当"]
    Deploy["デプロイメント・<br/>スモークテスト"]
    Done["納品準備完了"]

    SRS -->|"レビュー依頼"| SRS_Review
    SRS_Review -->|"Yes"| SWS
    SRS_Review -->|"No"| R1Fail
    R1Fail -->|"修正後"| SRS
    SWS -->|"レビュー依頼"| SWS_Review
    SWS_Review -->|"Yes"| Impl
    SWS_Review -->|"No"| R2Fail
    R2Fail -->|"修正後"| SWS
...
| 観点 | 内容 | 適用対象 |
|------|------|---------|
| **R1: 要件品質** | 完全性・テスト可能性・矛盾・曖昧表現・用語一貫性・アクセシビリティ(WCAG) | SRS |
| **R2: SW設計原則** | SOLID (SRP/OCP/LSP/ISP/DIP)、DRY、KISS、YAGNI、SoC、SLAP、LOD、CQS、POLA、PIE、CA、Naming | SWS・コード |
| **R3: コーディング品質** | エラーハンドリング完全性・入力バリデーション・防御的プログラミング | コード |
| **R4: 並行性・状態遷移** | デッドロック（リソース取得順序・長時間ロック）、レースコンディション（Check-Then-Act・DB Read-Modify-Write・Promise競合）、グリッジ（非原子更新・中間状態露出） | SWS・コード |
| **R5: パフォーマンス** | アルゴリズム計算量（O(n²)以上）、N+1クエリ、メモリリーク、不要な直列化 | SWS・コード |
...
## 解決したい課題
チーム内のタスク管理が属人化しており、進捗が可視化できていない。
...
## 主要機能
- タスクの作成、編集、削除
- タスクのステータス管理（Todo/InProgress/Done）
- 担当者のアサイン
- 期限の設定とリマインダー
- ダッシュボードでの進捗可視化
- チーム間のタスク共有
...
## 品質要求
- レスポンスタイム: P95 200ms以内（API）
- 同時接続: 100ユーザー
- 可用性: 99.9%
```

### 13.3 ステップ3: CLAUDE.md を配置

第3章のテンプレートに基づき、プロジェクト固有の情報を記入したCLAUDE.mdを配置する。ブランチ戦略・SAST/SCAツール・可観測性要件のセクションを必ず含めること。

### 13.4 ステップ4: エージェントとコマンドを配置

第4章のエージェント定義を `.claude/agents/` に、第5章のコマンドを `.claude/commands/` に配置する。特に **architect.md を忘れずに配置すること**（SWS・OpenAPI仕様生成に必須）。
...
1. Phase 0: spec.mdバリデーション → 条件付きプロセスを評価 → ユーザーに確認
2. Phase 1: spec.md を解析しSRSを作成 → R1レビュー → ユーザーにSRS承認を求める
3. Phase 2: SWS/OpenAPI仕様/セキュリティ設計/可観測性設計/WBSを並列作成 → 設計レビュー
...
ユーザーが関与するのは、Phase 0の確認・SRS承認・IaC適用承認・受入テスト（および途中で重要判断が必要な場合）のみである。
...
sequenceDiagram
    ...
    Lead->>SRS: SRS作成を依頼
    SRS->>Lead: SRS完成を報告
    Lead->>Review: SRSのR1レビューを依頼
    Review->>Lead: レビュー結果（PASS）を報告
    Lead->>User: SRS承認を要求
    User->>Lead: SRS承認

    par 並列設計フェーズ
        Lead->>Arch: SWS+OpenAPI作成を依頼
    ...
    Arch->>Lead: SWS+OpenAPI完成を報告
    Lead->>Review: SWSのR2/R4/R5レビューを依頼
    Review->>Lead: レビュー結果（PASS）を報告
...
| エージェント名 | 役割 | モデル | 区分 |
|--------------|------|--------|------|
| `architect` | SWS・OpenAPI仕様・マイグレーション設計 | opus | コア |
| `srs-writer` | 要求仕様書(SRS)の作成 | opus | コア |
