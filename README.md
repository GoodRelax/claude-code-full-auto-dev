...
setup フェーズで AI が CLAUDE.md を提案・配置した後、planning フェーズでユーザーへの構造化ヒアリングを実施し、srs-writer エージェントが `user-order.md` + ヒアリング結果 + `process-rules/spec-template-ja.md` を基に仕様書を `docs/spec/` に生成し、以降のフェーズが自動で進行します。
