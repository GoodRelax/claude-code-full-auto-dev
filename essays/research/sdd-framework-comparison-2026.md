...
> 調査日: 2026-03-16
> 対象: OpenSpec / Spec Kit / cc-sdd / claude-code-full-auto-dev
...
2026年現在、AI支援開発の品質と予測可能性を高める手法として **Spec-Driven Development（SDD）** が急速に台頭している。本報告書では、SDD領域の主要フレームワーク4種を多角的に比較し、④ claude-code-full-auto-dev の戦略的位置づけを明らかにする。
...
| ④ | claude-code-full-auto-dev | GoodRelax | 0 | MIT | 2026 |
...
### 2.4 ④ claude-code-full-auto-dev（GoodRelax）
...
```mermaid
flowchart TD
    Lead["lead<br/>オーケストレーター<br/>Opus"] -->|"フェーズ制御"| SRS["srs-writer<br/>仕様書Ch1-2<br/>Opus"]
    Lead -->|"フェーズ制御"| Arch["architect<br/>仕様書Ch3-6<br/>Opus"]
    Lead -->|"フェーズ制御"| Sec["security-reviewer<br/>脆弱性レビュー<br/>Opus"]
    Lead -->|"フェーズ制御"| Impl["implementer<br/>コード実装<br/>Opus"]
    Lead -->|"フェーズ制御"| Test["test-engineer<br/>テスト実行<br/>Sonnet"]
    Lead -->|"品質判定"| Rev["review-agent<br/>R1-R6レビュー<br/>Opus"]
    Lead -->|"進捗監視"| PM["progress-monitor<br/>WBS管理<br/>Sonnet"]
    Lead -->|"変更管理"| CM["change-manager<br/>CR処理<br/>Sonnet"]
    Lead -->|"リスク管理"| RM["risk-manager<br/>リスク台帳<br/>Sonnet"]
    Lead -->|"ライセンス確認"| LC["license-checker<br/>OSS互換性<br/>Haiku"]
    Lead -->|"用語チェック"| KK["kotodama-kun<br/>命名検証<br/>Haiku"]
    Rev -->|"Critical/Highで<br/>ブロック"| Lead
```

Lead エージェントを中心に、11の専門エージェントが役割分担する。モデルも Opus/Sonnet/Haiku の3段階で使い分ける。
...
**競合優位**: プロセス全体の自動化、12エージェント協調、6観点品質ゲート、不具合根本原因分析、用語統制
...
| 観点 | ① OpenSpec | ② Spec Kit | ③ cc-sdd | ④ full-auto-dev |
|------|-----------|-----------|---------|----------------|
| エージェントモデル | 単一エージェント | 単一エージェント | 単一 + サブエージェント | 12エージェント協調 |
| 役割分離 | なし | なし | 基本/サブエージェント | 12役割を明確分離 |
| モデル使い分け | ユーザー任意 | ユーザー任意 | ユーザー任意 | Opus/Sonnet/Haiku 最適配置 |
| エージェント間通信 | N/A | N/A | コマンド経由 | Lead 経由のハブ&スポーク |
...
## 第5章 ④ claude-code-full-auto-dev SWOT 分析
...
#### S2: 12エージェント協調アーキテクチャ

Lead エージェントを中心としたハブ&スポーク型のマルチエージェント協調は、現時点で他の SDD フレームワークには存在しない。さらに Opus/Sonnet/Haiku のモデル使い分けによるコスト最適化は実用的価値がある。
...
8つのプロセスルールブック、12のエージェント定義、22以上のファイルタイプ、条件付きプロセス評価マトリクスの理解が必要であり、導入の初期コストが極めて高い。①の「openspec init → /opsx:propose」や③の「npx cc-sdd」のようなワンコマンドスタートが存在しない。
...
**提案**: 12エージェント・8フェーズの完全構成とは別に、3エージェント・3フェーズの最小構成パッケージを提供する。
...
### 7.1 ④ claude-code-full-auto-dev の戦略的位置づけ
...
- claude-code-full-auto-dev: https://github.com/GoodRelax/claude-code-full-auto-dev
