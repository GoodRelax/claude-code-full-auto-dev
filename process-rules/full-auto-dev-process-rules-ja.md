...
- [第4章 開発ワークフロー](#第4章-開発ワークフロー)
  - 4.1 Phase 0: 条件付きプロセス評価（必須）
  - 4.2 Phase 1: 企画 — ヒアリングからANMS仕様書 Ch1-2 作成
  - 4.3 Phase 2: 設計 — ANMS仕様書 Ch3-6 詳細化・セキュリティ・WBS
  - 4.4 Phase 3: 実装 — 並列開発とテスト
  - 4.5 Phase 4: テスト — 統合テスト・性能テスト・品質監視
  - 4.6 Phase 5: 納品 — デプロイメント・最終レポート・受入テスト
...
| design | 2 | 設計 | `phase-design` |
| implementation | 3 | 実装 | `phase-implementation` |
| testing | 4 | テスト | `phase-testing` |
| delivery | 5 | 納品 | `phase-delivery` |
...
flowchart LR
    subgraph Phase0["Phase 0<br/>プロセス評価"]
        P0A["AI:<br/>user-order.mdバリデーション<br/>CLAUDE.md提案<br/>（言語設定含む）"]
        P0B["ユーザー:<br/>CLAUDE.md確認・承認<br/>言語・条件付きプロセス確認"]
    end

    subgraph Phase1["Phase 1<br/>企画"]
        P1A["ユーザー:<br/>コンセプト提示"]
        P1H["AI:<br/>構造化ヒアリング"]
        P1HU["ユーザー:<br/>ヒアリング確認"]
...
**出力:** `project-records/change-requests/change-request-{番号}.md`（変更要求票）、`project-records/change-requests/change-log.md`（変更管理台帳）
...
**リスク台帳フォーマット (project-records/risks/risk-register.json):**
...
**トレーサビリティマトリクスフォーマット (project-records/traceability/traceability-matrix.json):**
...
**出力:** `project-records/defects/DEF-{番号}.md`（障害票）：再現手順・深刻度・根本原因・再発防止策を含む
...
**リリース判定チェックリスト (project-records/release/release-checklist.md):**
...
- 廃止文書は `project-records/archive/` に移動し、廃止日と後継文書を記録する

---

### 3.4 条件付きプロセスの判断基準と判断時期

**判断タイミング:** Phase 0（user-order.md読み込み直後）にリードエージェントが評価し、該当するプロセスをCLAUDE.mdに追記して有効化する。
...
#### 3.4.2 Phase 0 評価プロセス

full-auto-dev コマンドのPhase 0で、リードエージェントが以下を自動評価する（第8章8.1参照）。

1. 機能安全 → 該当する場合は**即座にユーザーに確認**を求め、仕様書作成前に安全要件を確定する。`project-records/safety/` を作成し、Phase 2 に HARA・FMEAを追加する
2. 法規調査 → 該当する場合はCLAUDE.mdに追記し、ANMS仕様書 Ch2 の非機能要件セクションに規制要件を含める。`project-records/legal/` を作成する
3. 特許調査 → 該当する場合はWBSのPhase 2 開始前に特許調査タスクを追加する。`project-records/legal/patent-clearance.md` に記録する
...
1. docs/spec/ の ANMS 仕様書 Ch3 (Architecture) を詳細化する
...
**重要:** CLAUDE.md はユーザーが手動で記入するのではなく、**Phase 0 で AI が user-order.md を基に提案**する。ユーザーは提案内容を確認・承認するだけでよい。以下のテンプレートは AI が提案を生成する際の雛形である。
...
6. docs/spec/ の ANMS 仕様書 Ch3 (Architecture) を詳細化する
7. docs/spec/ の ANMS 仕様書 Ch4 (Specification) を Gherkin で詳細化する
8. docs/spec/ の ANMS 仕様書 Ch5 (Test Strategy) を定義する
9. docs/spec/ の ANMS 仕様書 Ch6 (Design Principles Compliance) を設定する
...
13. project-management/progress/wbs.md にWBSとガントチャートを作成する
...
24. 性能テストをANMS仕様書 Ch2 のNFR数値目標に基づき実行し、結果をproject-records/performance/に記録する
...
33. project-management/progress/final-report.md に最終レポートを作成する
34. 受入テスト手順書を作成する
35. ユーザーに完了報告する
...
`/full-auto-dev` を実行すると、Phase 0 で AI が user-order.md を読み込み、プロジェクトに適した CLAUDE.md を提案する。技術スタック・コーディング規約・セキュリティ方針・ブランチ戦略などが含まれるので、内容を確認して承認する。
...
1. Phase 0: user-order.mdバリデーション → CLAUDE.md提案 → 条件付きプロセスを評価 → ユーザーに確認
2. Phase 1: user-order.md + process-rules/spec-template-ja.md を基にANMS仕様書 Ch1-2 を作成 → R1レビュー → ユーザーに仕様書承認を求める
