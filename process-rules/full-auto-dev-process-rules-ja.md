...
flowchart TB
    ...
    Sec_Agent -->|"8 セキュリティ要求"| Impl_Agent
...
**注意:** change-request はユーザー起点の変更のみを扱う。AI側の技術的変更（障害修正、設計改善、依存変更）は defect または decision で管理する。
...
各フェーズ完了時、または障害多発時にふりかえりを実施し、改善内容をCLAUDE.mdとエージェント定義に反映する。文書管理規則（process-rules/）自体の改定が必要な場合はユーザー承認を要する。実行方法は第8章8.3のretrospectiveコマンドを参照。
...
| **機能安全（HARA/FMEA）**        | ・人命・身体への直接的影響がある（医療機器・車載・産業機器）<br>・IEC 61508 / ISO 26262 / IEC 62304 等への準拠が要求される<br>・社会インフラへの重大な影響がある<br>・金融基幹系で重大な資産損害リスクがある | setup フェーズ コンセプト提示時<br>（**最優先で判断**、安全要求は仕様書作成前に確定）      |
...
| **運用・保守** | ・本番環境でサービスを運用する<br>・リリース後の障害修正・パッチ適用が必要<br>・SLA（稼働率・応答時間）の保証が必要 | setup フェーズ<br>（operation フェーズの有効化判断） |

#### 3.4.2 setup フェーズ 評価プロセス

full-auto-dev コマンドの setup フェーズで、リードエージェントが以下を自動評価する（第8章8.1参照）。

1. 機能安全 → 該当する場合は**即座にユーザーに確認**を求め、仕様書作成前に安全要求を確定する。`project-records/safety/` を作成し、design フェーズに HARA・FMEAを追加する
...
flowchart TD
    ...
    fix["fix/DEF-001-login-error<br/>（障害修正）"]
...
1. 問題領域を特定（どのモジュール/機能に障害が集中しているか）
...
- **障害修正**: 本番で発見された不具合は defect（障害票）として記録し、修正→テスト→リリースのサイクルを回す
...
- **PM Agent**: project-management/progress/ に進捗レポートを出力する。WBS/障害カーブ/コストを管理する
...
エージェントの一覧・オーナーシップ・データフロー・フェーズ別アクティベーションは [エージェント一覧](agent-list-ja.md) を参照。

プロンプトの構造規約（S0-S6: Identity / Activation / Ownership / Procedure / Rules / Exception）は [プロンプト構造規約](prompt-structure-ja.md) を参照。

各エージェントのプロンプト定義は `.claude/agents/*.md` を参照。これが各エージェントの唯一の正（Single Source of Truth）である。

用語の定義・選定理由・略称の許可判定は [用語集](glossary-ja.md) を参照。
...
| testing | 障害オープン数（Critical/High） | 0件 | defect 集計 |
| testing | 障害カーブ収束 | 新規検出が減少傾向 | defect-curve.json |
...
6. testing: テスト消化曲線・障害カーブを監視しながら品質を確保 → 性能テスト → テストレビュー
