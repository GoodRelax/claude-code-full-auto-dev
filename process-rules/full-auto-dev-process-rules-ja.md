...
- [第4章 開発ワークフロー](#第4章-開発ワークフロー)
  ...
  - 4.2 planning フェーズ: 企画 — ヒアリングからANMS仕様書 Ch1-2 作成
  - 4.3 dependency-selection フェーズ: 外部依存の選定 — 評価・選定・調達（条件付き）
  - 4.4 design フェーズ: 設計 — ANMS仕様書 Ch3-6 詳細化・セキュリティ・WBS
...
| planning | 1 | 企画（ヒアリング＆仕様） | `phase-planning` |
...
flowchart TD
    subgraph Phase0["Phase 0: プロセス評価"]
        P0A["AI: user-order.mdバリデーション<br/>CLAUDE.md提案（言語設定含む）"]
        P0B["ユーザー: CLAUDE.md確認・承認<br/>言語・条件付きプロセス確認"]
        P0A -->|"評価結果報告"| P0B
    end

    subgraph Phase1["Phase 1: 企画"]
        P1A["ユーザー: コンセプト提示"]
        P1H["AI: 構造化ヒアリング<br/>（外部依存の識別含む）"]
        P1HU["ユーザー: ヒアリング確認"]
        P1B["AI: ANMS仕様書 Ch1-2作成"]
        P1C["AI: Ch1-2レビュー(R1)"]
        P1A -->|"コンセプト"| P1H
        P1H -->|"ヒアリング結果"| P1HU
        P1HU -->|"確認OK"| P1B
...
| **アクセシビリティ（WCAG 2.1）** | ・Webアプリケーションを提供する<br>・EU市場向け（EAA指令 2025年6月完全施行）<br>・公共・行政向けシステム<br>・多様なユーザー層を対象とする                                                                   | setup フェーズ 仕様書作成**前**<br>（NFRとしてANMS Ch2に含める）                            |
...
2. 法規調査 → 該当する場合はCLAUDE.mdに追記し、ANMS仕様書 Ch2 の非機能要件セクションに規制要件を含める。`project-records/legal/` を作成する
...
6. HW連携 → 該当する場合はCLAUDE.mdに追記し、planning フェーズのヒアリングにHW要件を含める。dependency-selection フェーズで外部依存の評価・選定を実施し、design フェーズで `docs/hardware/hw-requirement-spec.md` のCh3-6を完成させ、SW Spec Ch3 にAdapter層設計を追加する。testing フェーズにHW-SW結合テストを追加する
7. AI/LLM連携 → 該当する場合はCLAUDE.mdに追記し、planning フェーズのヒアリングにAI要件（能力・コスト・レイテンシ）を含める。dependency-selection フェーズで外部依存の評価・選定を実施し、design フェーズで `docs/ai/ai-requirement-spec.md` のCh3-6を完成させ、SW Spec Ch3 にAI Adapter層設計を追加する
...
バリデーション通過後、user-order.md の内容を基に **CLAUDE.md を提案**する（技術スタック、コーディング規約、セキュリティ方針、ブランチ戦略、**言語設定**など）。言語設定では以下を決定する:
...
- ヒアリング結果は `project-management/hearing-record.md` に構造化して記録する

#### 4.2.3 ユーザーのアクション: ヒアリング記録の確認

AIが作成したヒアリング記録をユーザーに提示し、以下を確認する:
...
#### 4.2.4 AI のアクション: ANMS仕様書 Ch1-2 自動生成

ヒアリング記録（hearing-record.md）と user-order.md を入力として、ANMS仕様書 Ch1-2 を作成する。

```bash
claude "user-order.mdを読み込み、ほぼ全自動開発を開始してください。
まず構造化ヒアリングを実施してください。
ヒアリング完了後、process-rules/spec-template-ja.md を参照し、
ANMS形式の仕様書（Ch1-2: Foundation・Requirements）を docs/spec/[project-name]-spec.md に作成してください。
作成後、review-agentでR1観点のレビューを実施し、
PASSしたら仕様書の概要を報告し、重要な判断が必要な箇所があれば提示してください。"
```

#### 4.2.5 ユーザーの判断ポイント: 仕様書承認
...
planning フェーズのヒアリングで識別された外部依存に対し、以下のステップを実施する。

#### 4.3.1 評価・選定プロセス

```bash
claude "ヒアリング結果に基づき、外部依存の評価・選定を実施してください:
1. ヒアリング記録から外部依存への要件を抽出し、requirement-specのドラフト（Ch1 目的、Ch2 要件）を作成する
...
Ch3 Architecture を詳細化する前に、planning フェーズのヒアリング結果（特に「ドメイン境界識別」）を入力として、プロジェクトの全コンポーネントを Clean Architecture の4層（Entity / Use Case / Adapter / Framework）に明示的に分類する。この仕訳結果が Ch3 のコンポーネント図・依存関係図の基礎となる。
...
1. docs/spec/ の ANMS 仕様書 Ch3 (Architecture) を詳細化する（レイヤー仕訳を先行して実施し、コンポーネントの4層分類を Ch3 冒頭に明記すること）
2. docs/spec/ の ANMS 仕様書 Ch4 (Specification) を Gherkin で詳細化する
3. docs/spec/ の ANMS 仕様書 Ch5 (Test Strategy) を定義する
4. docs/spec/ の ANMS 仕様書 Ch6 (Design Principles Compliance) を設定する
...
gantt
    ...
    section Phase3_設計
    ANMS Ch3-6詳細化               : p3a, after p2m, 3d
...
- R1指摘 → ANMS Ch1-2 修正（planning フェーズ相当）
- R2/R4/R5設計指摘 → ANMS Ch3-4 修正（design フェーズ相当）
...
project_root/
  ...
  process-rules/
    spec-template-ja.md           ... ANMS仕様書テンプレート（JA）
    spec-template-en.md           ... ANMS仕様書テンプレート（EN）
  essays/                         ... 論文・リサーチ（日英）
...
- **SRS Agent**: docs/spec/ に ANMS 形式の仕様書（Ch1-2: Foundation・Requirements）を作成。ユーザーコンセプトを構造化する
- **Architect Agent**: docs/spec/ の ANMS 仕様書 Ch3-6 を詳細化。docs/api/ にOpenAPI仕様を生成する
...
|-----------|------|------|--------------|
| planning フェーズ完了後（仕様書承認前） | ANMS Ch1-2 | R1 | ANMS Ch1-2 修正（planning フェーズへ戻る） |
| design フェーズ完了後（実装開始前） | ANMS Ch3-4・設計 | R2, R4, R5（設計レベル） | ANMS Ch3-4 修正（design フェーズへ戻る） |
...
- R1指摘（要件品質）: ANMS Ch1-2 修正が必要 → planning フェーズ相当の作業
- R2/R4/R5指摘（設計レベル）: ANMS Ch3-4 修正が必要 → design フェーズ相当の作業
...
2. process-rules/spec-template-ja.md を参照し、ANMS形式の仕様書を docs/spec/[project-name]-spec.md に作成する（Ch1-2: Foundation・Requirements）
...
6. ヒアリング記録から外部依存への要件を抽出し、requirement-specのドラフト（Ch1-2）を作成する
...
11. docs/spec/ の ANMS 仕様書 Ch3 (Architecture) を詳細化する
12. docs/spec/ の ANMS 仕様書 Ch4 (Specification) を Gherkin で詳細化する
13. docs/spec/ の ANMS 仕様書 Ch5 (Test Strategy) を定義する
14. docs/spec/ の ANMS 仕様書 Ch6 (Design Principles Compliance) を設定する
...
## Phase 4: 実装
21. ANMS仕様書に基づきsrc/にコードを実装する（Git worktreeで並列実装）
...
29. 性能テストをANMS仕様書 Ch2 のNFR数値目標に基づき実行し、結果をproject-records/performance/に記録する
...
flowchart TD
    ...
    R2Fail["ANMS Ch3-4 修正<br/>Phase 3相当"]
...
第7章のエージェント定義を `.claude/agents/` に、第8章のコマンドを `.claude/commands/` に配置する。特に **architect.md を忘れずに配置すること**（ANMS仕様書 Ch3-6 詳細化・OpenAPI仕様生成に必須）。
...
2. planning: user-order.md + process-rules/spec-template-ja.md を基にANMS仕様書 Ch1-2 を作成 → R1レビュー → ユーザーに仕様書承認を求める
3. dependency-selection: 外部依存の評価・選定（条件付き — HW連携・AI/LLM連携・フレームワーク要件定義が有効な場合）→ ユーザー承認
4. design: ANMS仕様書 Ch3-6 詳細化/OpenAPI仕様/セキュリティ設計/可観測性設計/WBSを並列作成 → 設計レビュー
