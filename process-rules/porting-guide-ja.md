...
本フレームワーク（claude-code-full-auto-dev）は Claude Code 上で構築されているが、フレームワークの本質的価値はプロセス規則（process-rules/）とプロンプト構造（S0-S6）にある。CLI固有のフロントマターやディレクトリ構造は「ガワ」であり、対象AIが自力で変換できる程度の差異である。
