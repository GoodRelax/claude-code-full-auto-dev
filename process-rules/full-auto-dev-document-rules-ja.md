...
{project-root}/
  ...
  docs/                           # 設計成果物（最終成果物）
    ...
    hardware/                     # HW要件仕様（条件付き）
    ai/                           # AI/LLM要件仕様（条件付き）
    framework/                    # フレームワーク要件仕様（条件付き）
...
| 災害復旧計画 | `dr-plan.md` | Common Block付き |
...
| ドラフト状態（draft/review/approved） | Form? Detail? | **Form Block** (`spec-foundation:draft_status`) | review-agentのゲート判断、change-managerのトリガー |
| 完成済みチャプター | Form? Detail? | **Form Block** (`spec-architecture:completed_chapters`) | architectが作業開始位置を判断 |
| 機能要件数 / 非機能要件数 | Form? Detail? | **Form Block** (`spec-foundation:fr_count`, `spec-foundation:nfr_count`) | traceabilityカバレッジ算出の母数 |
...
| NFR達成率（pass/total） | Form? Detail? | **Form Block** (`perf:nfr_pass_rate`) | 100%でないとtesting フェーズ不合格 |
...
| hw-requirement-spec | `hw-requirement-spec:` | HW要件仕様（条件付き。external-dependency-spec継承） | `docs/hardware/` | Yes |
| ai-requirement-spec | `ai-requirement-spec:` | AI/LLM要件仕様（条件付き。external-dependency-spec継承） | `docs/ai/` | Yes |
| framework-requirement-spec | `framework-requirement-spec:` | フレームワーク要件仕様（条件付き。external-dependency-spec継承） | `docs/framework/` | Yes |
...
| dr-plan | `dr-plan:` | 災害復旧計画（RPO/RTO・復旧手順） | `docs/operations/` | Yes |
| stakeholder-register | `stakeholder-register:` | ステークホルダー登録簿（推奨プロセス） | `project-management/` | Yes |

### external-dependency-spec（外部依存要件仕様テンプレート）

外部依存（Clean ArchitectureのFramework層）に対する要件仕様の共通テンプレート。以下の3つの具体的file_typeがこのテンプレートを継承する。
...
| dr-plan | `phase-design` | 運用チーム, lead | architect |
...
| `dr-plan:` | dr-plan Form Block | `<dr-plan:rto_hours>4</dr-plan:rto_hours>` |
...
| architect | hw-requirement-spec | HW要件仕様の完全制御（条件付きプロセス） |
| architect | ai-requirement-spec | AI/LLM要件仕様の完全制御（条件付きプロセス） |
| architect | framework-requirement-spec | フレームワーク要件仕様の完全制御（条件付きプロセス） |
