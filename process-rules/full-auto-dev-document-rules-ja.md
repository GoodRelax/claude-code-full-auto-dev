...
| ヒアリング記録 | `hearing-record.md` | シングルトン。planning フェーズで作成 |
...
**適用判断:** planning フェーズのヒアリングで「何が外部依存で、何が自分たちのオリジナルか」を識別する。I/F標準度が低い外部依存には専用のrequirement-specを作成する。I/F標準度が高い場合（標準SQL、公式フレームワークAPI等）は `project-records/decisions/` に選定理由を記録するのみで十分。
...
| hearing-record | `phase-planning` | srs-writer | srs-writer |
...
| srs-writer | hearing-record | ヒアリング記録の完全制御 |
