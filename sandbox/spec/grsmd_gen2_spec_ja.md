...
| ID     | 制限                                                                                                                                                                                                                                                      |
| ------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| LIM-01 | highlight.js は CDN の common バンドルを使用しているため、一部の言語（Dockerfile, Scala, PowerShell 等）のシンタックスハイライトが適用されない。EXT_TO_HLJS にマッピングは定義されているが、ブラウザコンソールに `Could not find the language` 警告が出る。 |
| LIM-02 | Edge ブラウザの「トラッキング防止」機能が CDN CSS に対して `Tracking Prevention blocked access to storage` 警告をコンソールに出す場合がある。機能への影響はない。                                                                                       |
...
const CONFIG = {
  ...
  keyHints: {
    markdown: "[Up][Down] : Scroll / [L] : Light   / [D] : Dark\n       [R] : Reload / [C] : Clear   / [N] : New Tab",
    code: "[Up][Down] : Scroll / [L] : Light   / [D] : Dark\n       [R] : Reload / [C] : Clear   / [N] : New Tab",
  },
  messages: {
    plantuml: {
